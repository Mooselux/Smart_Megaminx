...
uint8_t checkEncoders(){
  for (Encoder& e : encoders){
    ...
    uint8_t latestRead = (mcp[e.modul].digitalReadAda(e.pinA)<<1) | mcp[e.modul].digitalReadAda(e.pinB); 
...
void setup() {
  ...
  for (Encoder& e : encoders){
    mcp[e.modul].pinModeAda(e.pinA, INPUT);
    mcp[e.modul].pinModeAda(e.pinB, INPUT);
    e.lastRead = (mcp[e.modul].digitalReadAda(e.pinA)<<1) | mcp[e.modul].digitalReadAda(e.pinB);
  }
}
